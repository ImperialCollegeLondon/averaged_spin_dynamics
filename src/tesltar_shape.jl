#=
telstar401_shape.jl — approximate box-wing facet model for Telstar 401, built to
pair with the inertia model in `telstar401_inertia.jl`.
=#
using .master
using LinearAlgebra, Printf, StaticArrays

const ECHO_INERTIA_SCRIPT = false
const INERTIA_SCRIPT = joinpath(@__DIR__, "telstar401_inertia.jl")
...
# `mp` is the MassProperties from THAT script's own composite_inertia call
#  the reference the frame check below is made against.
const INERTIA_REF = Main.mp


# OPTICAL PROPERTIES - assumed
...
# Expected facet count: six bus box faces + front/back per wing.

const N_FACETS_EXPECTED = 6 + 2 * N_PANELS

# Base facets/primitives
...
function telstar401_shape(; array_angle_deg::Real = ARRAY_ANGLE_DEG,
    ...
    # Do not let this shape reach a torque calculation unannounced.  `maxlog`
    # keeps a sweep from drowning in it; `warn_inert = false` silences it for
    # callers that have read [FLAG-TELSTAR-INERT] and want the null baseline.
    if warn_inert
        @warn """
...
# checks the axis conjnction of the two models, inertia tensor and the facet model
...
and shares every geometry and mass constant with it.  Both are placeholders.""")
    ...
    # The check that would otherwise be invisible.
   ...
   *** THIS SHAPE IS DYNAMICALLY INERT. 
   """)
    ...
    println("""
GEOMETRY AND MASS ASSUMPTIONS ARE NOT REPEATED HERE.  Bus dimensions (borrowed
from Telstar 402), the bus dimension→axis map, total dry mass, bus/array mass
split, panel span/chord, boom offset and end-of-life array angle all come from
scripts/validation/telstar401_inertia.jl — run it to see its own ASSUMPTIONS
ledger, which is the single source of truth for those values.

NOTE: the ~3400 kg figure usually quoted for Telstar 401 is its LAUNCH mass;
TOTAL_DRY_MASS_KG = $(TOTAL_DRY_MASS_KG) kg is the assumed post-depletion dry mass, which is
what the rotational dynamics sees.  That value is itself a placeholder.

Nothing in either file is a measurement of Telstar 401.  Treat any torque or
spin-rate derived from this pair as a shape-class placeholder for sensitivity
work, not as a prediction for the real spacecraft.""")
